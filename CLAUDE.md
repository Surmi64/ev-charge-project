...
- The export follows the account's theme. `utils/reportTheme.js` resolves the palette and light/dark mode into the `--pr-*` properties the report block in `App.css` reads, and `utils/pdfExport.js` paints the same washed background the page carries onto every page. **Pagination is by block**: each `[data-pdf-block]` is placed whole or moved to the next page, so a chart is never split — which is why a chart and its table are separate sections rather than one. Only a block taller than a whole page is sliced.
